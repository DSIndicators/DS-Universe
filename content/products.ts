...
export const PRODUCTS: Product[] = [
    ...
    },
    {
        ...
        "description": "DS ProRSI runs a Wilder RSI and its signal line through a closed-bar crossover engine: a level is born on a midline, extreme, or any RSI/signal cross, anchored to the swing low or high of the last ten bars (or the last opposing candle), and spaced from its neighbours by an ATR gate so levels do not stack on top of each other. Each level is drawn as its own price-panel line — live while unbroken, tinted by the RSI's heat at birth, labelled with the exact RSI reading and a touch count that increments on every retest — and turns to a thin grey dashed trace the instant price closes through it (wick-break optional), with the last six live and forty broken levels kept. The DS panel underneath carries a header readout (RSI value plus state: BULLISH, OVERBOUGHT, OVERSOLD, COMPRESSION, BULL CROSS, BEAR CROSS), the nearest support and resistance distance in points and percent, a compression heat ribbon under the header, shaded 30/70 zones, and a segment-coloured RSI line running bull-to-neutral-to-bear over its signal. Compression is its own state — RSI and signal both within five points of the midline — so a genuinely quiet market reads as quiet rather than as a false cross. The panel's Y-axis locks by default, so dragging the chart can never push the reading off its own scale. Closed-bar decisions throughout: it does not repaint."
    },
    {
        "slug": "prostochastics",
    ...
    },
    {
        ...
        "purpose": "A MACD panel rebuilt on the volatility-normalised MACD-V scale, with the exact price that would cross the signal line solved in closed form before the bar even closes.",
        "hooks": [
    ...
    },
    {
        ...
        "helps": "On a NinjaTrader chart, price lives in one place: the axis, in small type, at the far right. DS Chart Price puts it front and centre instead, in a large readout you can place in any of nine spots on the panel, colour-coded so you can read direction and conviction from across the room — and it carries a small price-alert engine with built-in tones so you can step away and still hear when a level you set gets hit.",
        "description": "DS Chart Price renders the current price in large type at any of nine positions on the price panel (top, middle or bottom; left, centre or right). Every tick compares the new price to the last and flashes green or red, while choppiness is measured with the Kaufman Efficiency Ratio over a rolling window — low efficiency eases the resting colour toward amber, high efficiency holds a strong directional colour. The alert engine takes up to three price levels and plays that level's tone whenever price crosses it from either direction, with a re-arm distance in ticks and an optional cooldown so a level cannot repeat-fire while price hovers on it; alerts are realtime-only and never fire on historical bars, chart load or scrolling. Four tones are synthesised in memory with click-free envelopes — Chime, Bell, Pulse and the shared DS Universe tone — so there are no sound files to install, and a 0-100 volume control scales amplitude directly, true per-indicator loudness NinjaTrader's own alert playback cannot offer. Performance is deliberate: purely tick-driven with no forced refresh, zero per-frame allocation."
    },
    {
        "slug": "ds-258",
        "name": "DS 258",
        ...
        "hooks": [
            ...
            "Its own colour per level"
        ],
        "helps": "Nasdaq futures move in hundred-point blocks, and the same four prices inside every block keep doing the work: the 00, the 20, the 50 and the 80. Drawing them by hand gets old fast. DS 258 lays a whisper-quiet line on every one of them across the visible chart, in its own colour, at an opacity low enough to forget it's there — until price stops on one.",
        "description": "DS 258 is a pure render-only overlay: on every frame it reads the chart's own price scale and lays a line across the full width of the price panel at every 00, 20, 50 and 80 level currently in view — no calculation, no lookback, nothing to configure beyond which families to show. Each of the four levels carries its own DS Universe colour (gold for 00, blue for 20, platinum for 50, rose for 80) at a default opacity of 7, deliberately low enough that the map recedes into the chart until price actually reacts to one. Lines draw behind the candles by default and de-clutter automatically as you zoom out — the 20 and 80 families fade first, then the 50, so the chart never turns into a ladder of lines at wide zoom. Support in DS Toolkit covers all four level families from one row, including the mute and restore behaviour the rest of the suite uses."
    },
    {
        "slug": "parallax",
        "name": "DS Parallax",
